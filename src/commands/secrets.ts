import { confirm, intro, outro, isCancel, spinner } from "@clack/prompts";
...
export const secretCommands = ["test", "set", "migrate", "export"] as const;
export type SecretCommand = (typeof secretCommands)[number];
export const isSecretCommand = (command: string): command is SecretCommand =>
  secretCommands.includes(command as SecretCommand);

export default async (command: SecretCommand, args?: string[]) => {
...
async function setSecret(
  ...
  try {
    ...
    s.stop(
      `${green("✓")} ${key} stored securely using ${manager.getActiveBackendName()}`,
    );
  } catch (error) {
...
async function migrateSecrets(manager: SecretsManager): Promise<void> {
  ...
  const shouldContinue = await confirm({
    message:
      "This will move API keys from ~/.lazycommit to secure storage. Continue?",
    initialValue: true,
  });
  ...
  try {
    ...
    if (results.migrated.length === 0 && results.errors.length === 0) {
      ...
      console.log();
      outro(
        `Migration complete. Using: ${green(manager.getActiveBackendName() || "file")}`,
      );
    }
  } catch (error) {
...
async function exportSecrets(
  ...
  try {
    const { exportSecretsFromSecureStorage } = await import(
      "../utils/secrets/migrate.js"
    );
    const exportPath =
      outputPath || path.join(os.homedir(), ".lazycommit.backup");
